...
import { ChevronRight, Clapperboard, Group, Image as ImageIcon, Music2, RefreshCw, Star, Video } from "lucide-react";
...
function NodeContent(props: NodeContentRendererProps) {
    if ((props.node.type === CanvasNodeType.Config || props.node.type === CanvasNodeType.Creative) && props.renderNodeContent) return props.renderNodeContent(props.node);
...
const nodeContentRenderers = {
    [CanvasNodeType.Creative]: TextContent,
...
function DirectorNodeContent({ theme }: NodeContentRendererProps) {
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 p-5 text-center" style={{ color: theme.node.text }}>
            <span className="grid size-12 place-items-center rounded-lg" style={{ background: theme.toolbar.activeBg, color: "#14b8a6" }}>
                <Clapperboard className="size-6" />
            </span>
            <div>
                <div className="text-sm font-semibold">Santu Director</div>
                <div className="mt-1 text-xs" style={{ color: theme.node.muted }}>点击打开白模镜头预演</div>
            </div>
        </div>
    );
}
...
function ImageContent({
    ...
    if (loadFailed) {
        return <ErrorContent node={{ ...node, metadata: { ...node.metadata, errorDetails: "图片文件丢失，请重新上传或重新生成。" } }} theme={theme} onRetry={undefined} />;
    }
