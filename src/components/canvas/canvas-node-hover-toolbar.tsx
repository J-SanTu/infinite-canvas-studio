...
import { Download, Ellipsis, FolderPlus, Image as ImageIcon, Info, MessageSquare, Minus, Music2, Pencil, Plus, RefreshCw, Save, Settings2, Trash2, Upload, Video } from "lucide-react";
...
export function CanvasNodeHoverToolbar({
    ...
    const imageTools = buildImageToolbarTools(node, { onUpload, onToggleFreeResize, onMaskEdit, onOutpaint, onCrop, onSplit, onUpscale, onSuperResolve, onAngle, onViewImage, onCopyPrompt: copyImagePrompt, onReversePrompt });
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(hasImage && Boolean(node.metadata?.prompt?.trim()) ? [{ id: "savePrompt", title: "保存图片和提示词", label: "存提示词", icon: <Save className="size-4" />, onClick: () => onSavePrompt(node) }] : []),
    ...
    return (
        <>
            <div
                className="absolute z-[70] flex h-12 -translate-x-1/2 -translate-y-full items-center overflow-visible rounded-[18px] border text-[15px] shadow-[0_8px_28px_rgba(0,0,0,.18)] backdrop-blur-xl"
...
function ToolbarAction({ title, label, icon, onClick, showLabel, active = false, danger = false }: ToolbarTool & { showLabel: boolean }) {
    ...
    return (
        <Tooltip title={title} placement="top" mouseEnterDelay={0.2} color={theme.toolbar.panel} styles={{ root: { color: theme.toolbar.activeText, boxShadow: "0 8px 24px rgba(0,0,0,.20)", fontSize: 13, fontWeight: 500 } }}>
            <button
                type="button"
                className="group relative flex h-12 items-center whitespace-nowrap px-1.5"
                style={{ color: danger ? "#ef4444" : active ? theme.toolbar.activeText : theme.toolbar.item }}
                onClick={onClick}
                aria-label={title}
            >
                <span
                    className={`flex h-9 items-center ${hasText ? "gap-2 px-2.5" : "justify-center px-2"} rounded-lg transition group-hover:bg-[var(--canvas-toolbar-hover)]`}
                    style={{ "--canvas-toolbar-hover": theme.toolbar.itemHover, background: active ? theme.toolbar.activeBg : undefined } as CSSProperties}
                >
                    {icon}
