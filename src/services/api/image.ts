...
import { resizeDataUrlToExactSize } from "@/lib/canvas/canvas-image-data";
import { buildImageReferencePromptText } from "@/lib/image-reference-prompt";
import { imageToDataUrl } from "@/services/image-storage";
import type { ReferenceImage } from "@/types/image";
import { APP_DISABLE_RESPONSE_STORAGE, APP_REASONING_EFFORT } from "@/config/app-config";
...
async function requestOutputSizeSuperResolution(config: AiConfig, dataUrl: string, target: { width: number; height: number }, prompt: string, options?: RequestOptions) {
...
function readAxiosError(error: unknown, fallback: string) {
    ...
    if (axios.isAxiosError<{ error?: { message?: string } | string; msg?: string; code?: number }>(error)) {
        ...
        if (responseData?.msg || responseError || error.response?.status) return responseData?.msg || responseError || readStatusError(error.response?.status, fallback);
        return `${fallback}：${error.message || "网络错误"}。请确认本地服务仍在运行，并且 AI 请求正在通过 /api/ai 代理转发。`;
    }
...
function aiHeaders(config: AiConfig, contentType?: string) {
    return {
        ...
        ...(!config.baseUrl.includes("/api/ai") ? {} : { "X-Canvas-Capability": "text" }),
        ...(contentType ? { "Content-Type": contentType } : {}),
    };
}

function canvasImageRequestHeaders(baseUrl: string, model: string, quality: string | undefined, operation: "generation" | "edit", count: number) {
    if (!String(baseUrl || "").includes("/api/ai")) return {};
    return {
        "X-Canvas-Request-Id": `image_request_${nanoid()}`,
        "X-Canvas-Image-Operation": operation,
        "X-Canvas-Image-Model": model,
        "X-Canvas-Image-Quality": quality || "",
        "X-Canvas-Image-Count": String(Math.max(1, Math.floor(count) || 1)),
    };
}
...
async function requestStreamingResponse(config: AiConfig, body: Record<string, unknown>, onDelta?: (text: string) => void, options?: RequestOptions): Promise<ToolResponseResult> {
    const response = await fetch(aiApiUrl(config, "/responses"), {
        ...
        body: JSON.stringify({ ...body, reasoning: { effort: APP_REASONING_EFFORT }, store: !APP_DISABLE_RESPONSE_STORAGE, stream: true }),
        signal: options?.signal,
    });
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    try {
        const response = await axios.post<ImageApiResponse>(
            ...
            {
                headers: { ...aiHeaders(requestConfig, "application/json"), ...canvasImageRequestHeaders(requestConfig.baseUrl, requestConfig.model, quality || config.quality, "generation", n) },
                signal: options?.signal,
            },
        );
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    try {
        const response = await axios.post<ImageApiResponse>(aiApiUrl(requestConfig, "/images/edits"), formData, {
            headers: { ...aiHeaders(requestConfig), ...canvasImageRequestHeaders(requestConfig.baseUrl, requestConfig.model, quality || config.quality, "edit", n) },
            signal: options?.signal,
        });
        const images = parseImagePayload(response.data);
