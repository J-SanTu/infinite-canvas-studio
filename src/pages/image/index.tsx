...
export default function ImagePage() {
    ...
    const generate = async () => {
        ...
        for (let index = 0; index < generationCount; index += 1) {
            try {
                ...
                result.push({ status: "fulfilled", value: image });
            } catch (reason) {
        ...
        const failureMessage = failed?.reason instanceof Error ? failed.reason.message : failed ? "生成失败" : "";
    ...
    const saveResultPrompt = (image: GeneratedImage, index: number) => {
        const text = prompt.trim();
        if (!text) return message.warning("当前图片没有可保存的提示词");
        setPromptSaveSource({
    ...
    const previewGenerationLog = async (log: GenerationLog) => {
        ...
        if (!log.images.length && log.error) setResults([{ id: log.id, status: "failed", error: log.error }]);
    };
                        ...
                        <div className="mt-6 space-y-5">
                            ...
                            <div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="truncate text-stone-500 dark:text-stone-400">
                                    {modelOptionLabel(effectiveConfig, model)} · {imageSizeLabel(effectiveConfig.size)} · {imageQualityLabel(effectiveConfig.quality)}
                                </span>
...
function ResultImageCard({
    ...
    onSavePrompt: (image: GeneratedImage, index: number) => void;
}) {
    return (
        <div className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
            ...
            <div className="space-y-2 border-t border-stone-200 px-3 py-2.5 dark:border-stone-800">
                ...
                <div className="grid min-w-0 grid-cols-2 gap-2 xl:grid-cols-4">
                    ...
                    <Tooltip title="保存图片和提示词">
                        <Button className={RESULT_ACTION_BUTTON_CLASS} size="small" icon={<Save className="size-3.5" />} onClick={() => onSavePrompt(image, index)}>
                            保存提示词
                        </Button>
                    </Tooltip>
                    <Tooltip title="加入参考图">
                ...
                <div className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-start gap-2">
                    ...
                    <div className="min-w-0">
                        ...
                        {thumbnails.length ? (
                            ...
                            <div className="mt-1 line-clamp-2 text-xs leading-5 text-red-500">{log.error}</div>
                        ) : null}
                    </div>
                </div>
