...
import { pushAccountAssetLibrary } from "@/services/backend-asset-sync";
import { CANVAS_ASSETS_DRAG_MIME } from "@/lib/asset-drag";
import { cropDataUrl, outpaintDataUrl, splitDataUrl, upscaleDataUrl } from "@/lib/canvas/canvas-image-data";
...
import { CanvasNodeOutpaintDialog, type CanvasImageOutpaintParams } from "@/components/canvas/canvas-node-outpaint-dialog";
...
import { PromptSaveDialog } from "@/components/prompts/prompt-save-dialog";
...
import { getVisibleCanvasConnections, getVisibleCanvasNodes } from "@/lib/canvas/canvas-performance";
import { getCanvasLayoutSelection, layoutCanvasNodes, type CanvasAlignmentAction, type CanvasDistributionAction } from "@/lib/canvas/canvas-layout";
import type { DirectorCanvasReference } from "@/lib/director/director-project";
import { isDirectorReferenceImage } from "@/lib/image-reference-prompt";
import {
    CanvasNodeType,
...
import { deleteDirectorProject } from "@/services/api/director";
...
function InfiniteCanvasPage() {
    ...
    const assetAccountReady = useAssetStore((state) => state.accountReady);
    ...
    const [promptSaveSource, setPromptSaveSource] = useState<PromptSaveSource | null>(null);
    ...
    const handledDirectorAssetIdsRef = useRef(new Set<string>());
    ...
    const visibleNodes = useMemo(() => {
        const rect = containerRef.current?.getBoundingClientRect();
        return getVisibleCanvasNodes(nodes, viewport, { width: rect?.width || size.width, height: rect?.height || size.height }, { isHidden: (node) => isHiddenBatchChild(node, nodes, collapsingBatchIds) });
    }, [collapsingBatchIds, nodes, size.height, size.width, viewport]);
    ...
    const directorPanelNode = dialogNodeId ? nodes.find((node) => node.id === dialogNodeId && node.type === CanvasNodeType.Director) || null : null;
    const directorCanvasReferences = useMemo<DirectorCanvasReference[]>(() => {
        if (!directorPanelNode) return [];
        return buildNodeGenerationInputs(directorPanelNode.id, nodes, connections).map((input) => ({
            nodeId: input.nodeId,
            kind: input.type,
            title: input.title,
    ...
    }, [connections, directorPanelNode, nodes]);
    const hasMultipleSelectedNodes = selectedNodeIds.size > 1;
    const selectedLayoutItemCount = useMemo(() => getCanvasLayoutSelection(nodes, selectedNodeIds).length, [nodes, selectedNodeIds]);
    ...
    const visibleConnections = useMemo(
        () => getVisibleCanvasConnections(connections, nodeById, visibleNodeIds, (node) => isHiddenBatchConnectionEndpoint(node, nodeById), relatedHighlight.connectionIds),
        [connections, nodeById, relatedHighlight.connectionIds, visibleNodeIds],
    );
    ...
    const deleteNodes = useCallback(
        (ids: Set<string>) => {
            ...
            nodesRef.current
                .filter((node) => allIds.has(node.id) && node.type === CanvasNodeType.Director)
                .forEach((node) => {
                    void deleteDirectorProject(node.id).catch((error) => {
                        message.warning(error instanceof Error ? error.message : "导演工程清理失败");
                    });
                });
            setNodes((prev) => {
    ...
    const handleDirectorExport = useCallback(
        async ({ asset, run }: { asset: Asset; run: GenerationRun; blob: Blob }) => {
            const directorNode = nodesRef.current.find((node) => node.id === run.targetNodeId) || directorPanelNode;
            if (!directorNode || (asset.kind !== "image" && asset.kind !== "video")) return;

            useAssetStore.setState((state) => ({ assets: [asset, ...state.assets.filter((item) => item.id !== asset.id)] }));
            const isImage = asset.kind === "image";
            const directorProjectId = typeof asset.metadata?.directorProjectId === "string" ? asset.metadata.directorProjectId : directorNode.metadata?.directorProjectId;
            const sourceNodeIds = Array.from(new Set([directorNode.id, ...(run.sourceNodeIds || [])]));
            setNodes((prev) => prev.map((item) => (item.id === directorNode.id ? { ...item, metadata: { ...item.metadata, directorProjectId, directorRunId: run.id, previewAssetId: asset.id, directorSourceNodeIds: sourceNodeIds } } : item)));
            const existingOutput = nodesRef.current.find((node) => node.metadata?.previewAssetId === asset.id);
            if (handledDirectorAssetIdsRef.current.has(asset.id) || existingOutput) {
                handledDirectorAssetIdsRef.current.add(asset.id);
                if (existingOutput) setSelectedNodeIds(new Set([existingOutput.id]));
                setDialogNodeId(null);
                return;
            }
            handledDirectorAssetIdsRef.current.add(asset.id);
            const content = isImage ? await resolveImageUrl(asset.data.storageKey, asset.data.dataUrl) : await resolveMediaUrl(asset.data.storageKey, asset.data.url);
            const width = Math.max(1, asset.data.width || 1024);
            const height = Math.max(1, asset.data.height || 1024);
            const outputNode = createCanvasNode(isImage ? CanvasNodeType.Image : CanvasNodeType.Video, {
                x: directorNode.position.x + directorNode.width + 48 + Math.min(width, 420) / 2,
                y: directorNode.position.y + directorNode.height / 2,
            });
            const fitted = fitNodeSize(width, height, isImage ? 420 : VIDEO_NODE_MAX_WIDTH, isImage ? 420 : VIDEO_NODE_MAX_HEIGHT);
            outputNode.title = asset.title;
            outputNode.width = fitted.width;
            outputNode.height = fitted.height;
            outputNode.metadata = {
                ...outputNode.metadata,
                content,
                ...
                durationMs: isImage ? undefined : asset.data.durationMs,
                directorProjectId,
                directorRunId: run.id,
                previewAssetId: asset.id,
                directorSourceNodeIds: sourceNodeIds,
            };

            setNodes((prev) => [...prev, outputNode]);
            setConnections((prev) => [...prev, { id: nanoid(), fromNodeId: directorNode.id, toNodeId: outputNode.id }]);
    ...
    const applyCanvasAlignment = useCallback(
        (action: CanvasAlignmentAction) => {
            setDialogNodeId(null);
            setToolbarNodeId(null);
            setNodes((current) => layoutCanvasNodes(current, selectedNodeIds, action));
        },
        [selectedNodeIds],
    );

    const applyCanvasDistribution = useCallback(
        (axis: CanvasDistributionAction) => {
            setDialogNodeId(null);
            setToolbarNodeId(null);
            setNodes((current) => layoutCanvasNodes(current, selectedNodeIds, `distribute-${axis}`));
        },
        [selectedNodeIds],
    );
    ...
    const createImportedFileNode = useCallback(async (file: File, position: Position): Promise<CanvasNodeData> => {
        if (isAudioFile(file)) {
            const audio = await uploadMediaFile(file, "audio");
            const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Audio];
            return {
    ...
    const createImportedFileNodes = useCallback(
        async (files: File[], position: Position) => {
            const supportedFiles = files.filter(isSupportedCanvasFile);
            if (!supportedFiles.length) return;

            const importedNodes = await Promise.all(supportedFiles.map((file) => createImportedFileNode(file, position)));
            let top = position.y - importedNodes[0].height / 2;
            const laidOutNodes = importedNodes.map((node) => {
                const nextNode = {
                    ...node,
                    position: {
                ...
                top += node.height + IMPORT_NODE_GAP;
                return nextNode;
            });
            const importedIds = laidOutNodes.map((node) => node.id);

            setNodes((prev) => [...prev, ...laidOutNodes]);
            setSelectedNodeIds(new Set(importedIds));
            setSelectedConnectionId(null);
            setDialogNodeId(importedIds.at(-1) || null);
        },
    ...
    const importClipboardImages = useCallback(
        (files: File[], position = getCanvasCenter()) => {
            const images = files.filter((file) => file.type.startsWith("image/"));
            if (!images.length) return false;
            void createImportedFileNodes(images, position)
                .then(() => message.success(images.length > 1 ? `已从剪切板添加 ${images.length} 张图片` : "已从剪切板添加图片"))
                .catch((error) => message.error(error instanceof Error ? error.message : "剪切板图片添加失败"));
            return true;
        },
        [createImportedFileNodes, getCanvasCenter, message],
    );
    ...
    useEffect(() => {
        const handlePaste = (event: ClipboardEvent) => {
            ...
            if (files.length) {
                event.preventDefault();
                importClipboardImages(files);
                return;
            }

            const text = event.clipboardData?.getData("text/plain") || "";
            if (createTextNodeFromClipboard(text)) event.preventDefault();
        };

        window.addEventListener("paste", handlePaste);
        return () => window.removeEventListener("paste", handlePaste);
    }, [createTextNodeFromClipboard, importClipboardImages]);
    ...
    const saveNodeAsset = useCallback(
        async (node: CanvasNodeData) => {
            if (!assetAccountReady) return message.warning("请等待本地素材加载完成");
            if (node.type !== CanvasNodeType.Image) return message.warning("本地素材当前支持保存图片节点");
            if (!node.metadata?.content) return message.error("没有可保存的图片");
            const storedImage = node.metadata.storageKey
                ? {
                ...
                : await uploadImage(node.metadata.content);
            const assetId = addAsset({
                kind: "image",
                title: node.title || node.metadata?.prompt?.slice(0, 24) || "画布图片",
                coverUrl: storedImage.url,
                tags: [],
                folderId: null,
                source: "Canvas",
                data: { dataUrl: storedImage.url, storageKey: storedImage.storageKey, width: storedImage.width, height: storedImage.height, bytes: storedImage.bytes, mimeType: storedImage.mimeType },
                metadata: { source: "canvas", nodeId: node.id, prompt: node.metadata?.prompt },
            });
            try {
                const state = useAssetStore.getState();
                await pushAccountAssetLibrary(state.assets, state.folders);
                message.success("已保存到本地素材");
            } catch (error) {
                removeAsset(assetId);
                message.error(error instanceof Error ? error.message : "素材保存失败");
            }
        },
        [addAsset, assetAccountReady, message, removeAsset, user],
    );

    const saveNodePrompt = useCallback(
        (node: CanvasNodeData) => {
            const prompt = node.metadata?.prompt?.trim() || "";
            if (node.type !== CanvasNodeType.Image || !node.metadata?.content || !prompt) {
                message.warning("当前图片没有可保存的提示词");
                return;
            }
            setPromptSaveSource({
                imageUrl: node.metadata.content,
                imageStorageKey: node.metadata.storageKey,
                imageWidth: node.metadata.naturalWidth || node.width,
                imageHeight: node.metadata.naturalHeight || node.height,
                imageBytes: node.metadata.bytes,
                imageMimeType: node.metadata.mimeType,
                prompt,
                title: node.title || Array.from(prompt).slice(0, 24).join(""),
                sourceType: "canvas",
                sourceRefId: node.id,
            });
        },
    ...
    const outpaintImageNode = useCallback(
        async (node: CanvasNodeData, params: CanvasImageOutpaintParams) => {
            if (!node.metadata?.content) return;
            let outpaint: Awaited<ReturnType<typeof outpaintDataUrl>>;
            try {
                outpaint = await outpaintDataUrl(node.metadata.content, params);
            } catch (error) {
                const errorDetails = error instanceof Error ? error.message : "扩图尺寸不支持";
                message.error(errorDetails);
                return;
            }
            const generationConfig = { ...buildGenerationConfig(effectiveConfig, node, "image"), count: "1", size: `${outpaint.width}x${outpaint.height}` };
            if (!isAiConfigReady(generationConfig, generationConfig.model)) {
                openConfigDialog(true);
                return;
            }
            const userPrompt = params.prompt.trim();
            const prompt = `只生成并补全透明扩展区域，必须保持框内原图内容、构图、主体、文字、产品和边缘不变。扩展区域需要自然延续原图的场景、光线、透视、材质、颜色和景深。${userPrompt}`;
            const childId = nanoid();
            const source = { id: `${node.id}-outpaint-source`, name: `${node.title || node.id}-outpaint.png`, type: "image/png", dataUrl: outpaint.imageDataUrl };
            const mask = { id: `${node.id}-outpaint-mask`, name: "outpaint-mask.png", type: "image/png", dataUrl: outpaint.maskDataUrl };
            const generationMetadata = buildImageGenerationMetadata("edit", generationConfig, 1, [source]);
            const displaySize = fitNodeSize(outpaint.width, outpaint.height);
            ...
            const controller = startGenerationRequest(childId, node.id, childId);
            try {
                const image = await requestOutpaintEditWithFallback(generationConfig, prompt, source, mask, { signal: controller.signal });
                const uploaded = await uploadImage(image.dataUrl);
    ...
    const handleImageInputChange = useCallback(
        async (event: ReactChangeEvent<HTMLInputElement>) => {
            ...
            if (target?.nodeId) {
                ...
                void createImportedFileNodes(files, position);
            }

            uploadTargetRef.current = null;
            event.target.value = "";
        },
        [createImportedFileNodes, screenToCanvas, size.height, size.width],
    );

    const insertAssetPayloads = useCallback(
        async (payloads: InsertAssetPayload[], anchor?: Position) => {
            if (!payloads.length) return;
            try {
                const base = anchor || screenToCanvas((containerRef.current?.getBoundingClientRect().left || 0) + size.width / 2, (containerRef.current?.getBoundingClientRect().top || 0) + size.height / 2);
                const created: CanvasNodeData[] = [];
                const stamp = Date.now();

                for (let index = 0; index < payloads.length; index += 1) {
                    const payload = payloads[index];
                    const center = { x: base.x + (index % 2) * 380, y: base.y + Math.floor(index / 2) * 340 };
                    if (payload.kind === "text") {
                        const node = { ...createCanvasNode(CanvasNodeType.Text, center, { content: payload.content, status: NODE_STATUS_SUCCESS }), title: payload.title || payload.content.slice(0, 32) || "素材文本" };
                        created.push(node);
                        continue;
                    }
                    ...
                    if (payload.kind === "audio") {
                        const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Audio];
                        created.push({
                            id: `audio-${stamp}-${index}`,
                            type: CanvasNodeType.Audio,
                            title: payload.title,
                            position: { x: center.x - spec.width / 2, y: center.y - spec.height / 2 },
                            width: spec.width,
                            height: spec.height,
                            metadata: { content: payload.url, storageKey: payload.storageKey, status: NODE_STATUS_SUCCESS, durationMs: payload.durationMs, mimeType: "audio/mpeg" },
                        });
                        continue;
                    }
                    ...
                    created.push({
                        id: `image-${stamp}-${index}`,
                        type: CanvasNodeType.Image,
                        title: payload.title || "素材图片",
                        position: { x: center.x - config.width / 2, y: center.y - config.height / 2 },
                        width: config.width,
                        height: config.height,
                        metadata: { ...imageMetadata({ ...storedImage, width: meta.width, height: meta.height }), prompt: payload.title },
                    });
                }
                ...
                message.error(error instanceof Error ? error.message : "素材添加到画布失败");
            }
        },
    ...
    const handleDrop = useCallback(
        (event: ReactDragEvent<HTMLDivElement>) => {
            event.preventDefault();
            const draggedAssets = event.dataTransfer.getData(CANVAS_ASSETS_DRAG_MIME);
            if (draggedAssets) {
                try {
                    const payloads = JSON.parse(draggedAssets) as InsertAssetPayload[];
                    if (Array.isArray(payloads) && payloads.length) {
                        void insertAssetPayloads(payloads, screenToCanvas(event.clientX, event.clientY));
                        return;
                    }
                } catch {
                    message.error("拖入的素材数据无效");
                    return;
                }
            }
            ...
            void createImportedFileNodes(files, pos);
        },
        [createImportedFileNodes, insertAssetPayloads, message, screenToCanvas],
    );
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                if (mode === "image") {
                            ...
                            ? [
                                  {
                                      ...
                                      isDirectorReference: isDirectorReferenceImage({
                                          isDirectorReference: Boolean(sourceNode.metadata.directorReferenceLock || sourceNode.metadata.directorProjectId || sourceNode.metadata.directorSourceNodeIds?.length),
                                          storageKey: sourceNode.metadata.storageKey,
                    ...
                    const childrenHeight = childIds.length * imageConfig.height + Math.max(0, childIds.length - 1) * rowGap;
                    const childrenStartY = rootNode.position.y + rootNode.height / 2 - childrenHeight / 2;
                    const childNodes: CanvasNodeData[] = childIds.map((id, index) => ({
                        ...
                        position: {
                            ...
                            y: childrenStartY + index * (imageConfig.height + rowGap),
                        },
                        width: imageConfig.width,
                        height: imageConfig.height,
                    ...
                    for (const targetId of targetIds) {
                        ...
                        try {
                            ...
                            const errorDetails = error instanceof Error ? error.message : "生成失败";
                            hasFailure = true;
                            if (!firstErrorDetails) firstErrorDetails = errorDetails;
                    ...
                    const batchErrorDetails = firstErrorDetails || "全部图片生成失败";
                    if (hasFailure) message.error(hasSuccess ? `部分图片生成失败：${batchErrorDetails}` : batchErrorDetails);
                    setNodes((prev) =>
                        prev.map((node) =>
                            node.id === nodeId && isConfigNode
                                ? { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : batchErrorDetails } }
                                : node.id === nodeId && isEmptyImageNode
                                  ? { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : batchErrorDetails } }
                                  : node.id === rootId && !hasSuccess
                                    ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, errorDetails: batchErrorDetails } }
                                    : node,
                        ),
                    );
                    return;
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            const liveReferenceContext = useReferenceImages ? await hydrateNodeGenerationContext(buildNodeGenerationContext(sourceNode.type === CanvasNodeType.Config ? sourceNode.id : node.id, nodesRef.current, connectionsRef.current, "")) : null;
            const liveReferenceImages = liveReferenceContext?.referenceImages.length ? liveReferenceContext.referenceImages : sourceNodeReferenceImages(batchRoot || sourceNode);
            const retryReferenceImages =
                hasSavedImageMetadata && savedImageMetadata ? (await resolveMetadataReferences(savedImageMetadata)) || liveReferenceImages : useReferenceImages ? (context?.referenceImages.length ? context.referenceImages : liveReferenceImages) : [];
            if (useReferenceImages && retryReferenceImages.length === 0) {
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                {directorPanelNode ? <DirectorPanel nodeId={directorPanelNode.id} canvasProjectId={projectId} references={directorCanvasReferences} open onClose={() => setDialogNodeId(null)} onExport={handleDirectorExport} /> : null}
                ...
                <PromptSaveDialog
                    open={Boolean(promptSaveSource)}
                    source={promptSaveSource}
                    onOpenChange={(next) => {
                        if (!next) setPromptSaveSource(null);
                    }}
                />

                <CanvasToolbar
                    selectedCount={selectedNodeIds.size}
                    layoutItemCount={selectedLayoutItemCount}
                    canUndo={historyState.canUndo}
                    canRedo={historyState.canRedo}
                    backgroundMode={backgroundMode}
                    showImageInfo={showImageInfo}
                    ...
                    onAlign={applyCanvasAlignment}
                    onDistribute={applyCanvasDistribution}
                />
                ...
                <input ref={imageInputRef} type="file" accept="image/*,video/*,audio/mpeg,audio/wav,audio/x-wav,.mp3,.wav" className="hidden" multiple onChange={handleImageInputChange} />
                ...
                {outpaintNode?.metadata?.content ? (
                    <CanvasNodeOutpaintDialog dataUrl={outpaintNode.metadata.content} open={Boolean(outpaintNode)} onClose={() => setOutpaintNodeId(null)} onConfirm={(params) => void outpaintImageNode(outpaintNode!, params)} />
                ) : null}
                ...
                <AssetPickerModal open={assetPickerOpen} onInsert={(payload) => void insertAssetPayloads([payload])} onInsertMany={(payloads) => void insertAssetPayloads(payloads)} onClose={() => setAssetPickerOpen(false)} />
            </section>
        </main>
    );
}
...
function CanvasTopBar({
    ...
    return (
        <>
            ...
            <Modal title="快捷键" open={shortcutsOpen} onCancel={() => setShortcutsOpen(false)} footer={null} centered styles={{ body: { maxHeight: "calc(100vh - 150px)", overflowY: "auto" } }}>
                <div className="space-y-2 border-t pt-4 text-sm" style={{ borderColor: theme.node.stroke }}>
                    <Shortcut keys={["鼠标中键", "拖动"]} value="平移视图" />
                    <Shortcut keys={["Mac 双指"]} value="平移视图" />
                    <Shortcut keys={["触控板捏合"]} value="缩放画布" />
                    <Shortcut keys={["缩放滑杆"]} value="精确调整缩放" />
                    <Shortcut keys={["左键拖动"]} value="框选多个节点" />
                    <Shortcut keys={["Shift", "左键拖动"]} value="追加框选节点" />
...
function buildImageGenerationMetadata(type: CanvasImageGenerationType, config: AiConfig, count: number, references: ReferenceImage[]): CanvasNodeMetadata {
    return {
        ...
        directorReferenceLock: references.some((reference) => reference.isDirectorReference),
    };
}
...
async function requestOutpaintEditWithFallback(config: AiConfig, prompt: string, source: ReferenceImage, mask: ReferenceImage, options: { signal?: AbortSignal }) {
    try {
        return await requestEdit(config, prompt, [source], mask, options).then((items) => items[0]);
    } catch (error) {
        if (!isMaskServerError(error)) throw error;
        const fallbackPrompt = `${prompt}\n\n如果请求包含透明区域，请只补全透明扩展区域，保持已有非透明原图完全不变。`;
        return requestEdit(config, fallbackPrompt, [source], undefined, options).then((items) => items[0]);
    }
}

function isMaskServerError(error: unknown) {
    return error instanceof Error && /请求失败[:：]\s*500|HTTP 500|500/.test(error.message);
}
...
async function resolveMetadataReferences(metadata: CanvasNodeMetadata) {
    ...
    const directorReferenceLock = Boolean(metadata.directorReferenceLock || metadata.references.some((url) => url.startsWith("image:director_asset_")));
    const references = await Promise.all(
        metadata.references.map(async (url, index) => {
            const dataUrl = url.startsWith("image:") ? await resolveImageUrl(url, "") : url;
            return dataUrl
                ? { id: `${index}`, name: `reference-${index}.png`, type: "image/png", dataUrl, storageKey: url.startsWith("image:") ? url : undefined, isDirectorReference: directorReferenceLock || url.startsWith("image:director_asset_") }
                : null;
        }),
    );
    ...
    return Promise.all(
        nodes.map(async (node) => {
            ...
            if (node.metadata?.storageKey) {
                const restored = await resolveImageUrl(node.metadata.storageKey, "");
                if (!restored) return { ...node, metadata: { ...node.metadata, content: "", status: NODE_STATUS_ERROR, errorDetails: "图片文件丢失，请重新上传或重新生成。" } };
                return { ...node, metadata: { ...node.metadata, content: restored, status: node.metadata.status === NODE_STATUS_ERROR ? NODE_STATUS_SUCCESS : node.metadata.status, errorDetails: undefined } };
            }
    ...
    return [
        {
            ...
            storageKey: node.metadata.storageKey,
            isDirectorReference: isDirectorReferenceImage({
                isDirectorReference: Boolean(node.metadata.directorReferenceLock || node.metadata.directorProjectId || node.metadata.directorSourceNodeIds?.length),
                storageKey: node.metadata.storageKey,
                url: node.metadata.content,
            }),
        },
...
function isHiddenBatchConnectionEndpoint(node: CanvasNodeData, nodes: CanvasNodeData[] | ReadonlyMap<string, CanvasNodeData>) {
    const rootId = node.metadata?.batchRootId;
    if (!rootId) return false;
    const root = Array.isArray(nodes) ? findCanvasNode(nodes, rootId) : nodes.get(rootId);
    return Boolean(root && !root.metadata?.imageBatchExpanded);
}

function findCanvasNode(nodes: CanvasNodeData[], id: string) {
    return nodes.find((item: CanvasNodeData) => item.id === id);
}
