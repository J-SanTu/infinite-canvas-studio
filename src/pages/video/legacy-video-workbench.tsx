...
export default function VideoPage() {
    ...
    const generate = async () => {
        ...
        const log = buildLog({ prompt: snapshot.text, model, config: snapshot.config, references: snapshot.references, videoReferences: snapshot.videoReferences, audioReferences: snapshot.audioReferences, durationMs: 0, status: "生成中" });
        let runId = "";
        try {
            const run = await createGenerationRun({
                requestId: `video_${log.id}`,
                operation: "video_generation",
                capability: "video",
                modelId: model,
                inputSnapshot: { prompt: snapshot.text, referenceImageCount: snapshot.references.length, referenceVideoCount: snapshot.videoReferences.length, referenceAudioCount: snapshot.audioReferences.length },
                parameterSnapshot: { size: snapshot.config.size, vquality: snapshot.config.vquality, videoSeconds: snapshot.config.videoSeconds, videoGenerateAudio: snapshot.config.videoGenerateAudio, videoWatermark: snapshot.config.videoWatermark },
            });
            runId = run.id;
            log.runId = run.id;
            await saveLog(log);
            ...
            await saveLog({ ...log, runId: runId || undefined, durationMs: performance.now() - batchStartedAt, status: "失败", error: errorMessage });
            if (runId) {
                const errorCode = classifyVideoError(errorMessage);
                await transitionGenerationRun(runId, isRetryableVideoError(errorCode) ? "retryable_failed" : "failed", { errorCode, errorMessageSafe: errorMessage }).catch(() => undefined);
            }
    ...
    const saveResultToAssets = (video: GeneratedVideo) => {
        ...
        const log = logs.find((item) => item.video?.id === video.id || item.video?.storageKey === video.storageKey);
        if (log?.runId) void transitionGenerationRun(log.runId, "succeeded", { outputAssetIds: [assetId] }).catch(() => undefined);
        ...
        try {
            for (let attempt = 0; attempt < 120; attempt += 1) {
                ...
                if (state.status === "completed") {
                    ...
                    if (log.runId) await transitionGenerationRun(log.runId, "succeeded");
            ...
            if (log.runId) {
                const errorCode = classifyVideoError(errorMessage);
                await transitionGenerationRun(log.runId, isRetryableVideoError(errorCode) ? "retryable_failed" : "failed", { errorCode, errorMessageSafe: errorMessage }).catch(() => undefined);
            }
            message.error(errorMessage);
        } finally {
...
function classifyVideoError(message: string) {
...
function isRetryableVideoError(code: string) {
