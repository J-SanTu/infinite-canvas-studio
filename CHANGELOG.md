...
- 新增“创意策划”图片节点：输入产品图、风格参考图、产品链接与要求，生成画面描述和逐条文案。
- 文案独立编辑并实时同步最终 Prompt，下游继承最新方案和两类参考图。
- 增加比例冲突确认：显示双方比例，支持自动改用策划比例或确认下游比例风险。
- 新增“确认生成图片”，自动创建、连接图片节点并启动生成。
- 改善按钮与下拉框状态、模块间距、多行文案、滚轮及触控板滚动。
- 增加运行阶段、流动光晕进度指示和复制成功反馈。
- 保留此前导演台保存、摄像机控制和图片不裁切修复。

验证：类型检查、生产构建、创意策划与图片比例回归测试。真实模型生成及设备交互仍需使用者验收；Mac 未进行 Apple 公证。
